...
const Carousel = ({photo}: Curbside)=> {
    ...
    return(
        <div className="relative">
            <button className={page === 0 ? "hidden" : ""} onClick={()=>{ page > 0 ? setPage(page - 1) : setPage(photo.length - 1); setPreviousPage(page); setLoading(true);}}>
                <i className="absolute text-white text-2xl transition top-[7rem] left-[.5rem] md:top-[13rem] fa fa-arrow-circle-left hover:scale-110 hover:text-black z-2"></i>
            </button>
            <button className={page === (photo.length - 1) ? "hidden" : ""} onClick={()=>{ page < (photo.length - 1) ? setPage(page + 1) : setPage(0); setPreviousPage(page); setLoading(true);}}>
                <i className="absolute text-white text-2xl transition top-[7rem] right-[.5rem] md:top-[13rem] fa fa-arrow-circle-right hover:scale-110 hover:text-black z-2"></i>
            </button>
